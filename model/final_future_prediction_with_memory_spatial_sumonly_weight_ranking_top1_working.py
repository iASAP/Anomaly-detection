...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys,train=True, x2=None):
        ...
        if x2 != None:
            ...
            fea_2, skip1_2, skip2_2, skip3_2 = self.encoder(x2)
            updated_fea2, keys_2, softmax_score_query_2, softmax_score_memory_2,query_2, top1_keys_2, keys_ind_2, compactness_loss_2 = self.memory(fea_2, keys, train)

            # Compute feature diff.
            # fea_diff = updated_fea - updated_fea2
            # skip1_diff = skip1 - skip1_2
            # skip2_diff = skip2 - skip2_2
            # skip3_diff = skip3 - skip3_2
            # fea_diff = torch.sqrt((updated_fea - updated_fea2)**2)
            # skip1_diff = torch.sqrt((skip1 - skip1_2)**2)
            # skip2_diff = torch.sqrt((skip2 - skip2_2)**2)
            # skip3_diff = torch.sqrt((skip3 - skip3_2)**2)
            fea_diff = (updated_fea - updated_fea2).pow(2)
            skip1_diff = (skip1 - skip1_2).pow(2)
            skip2_diff = (skip2 - skip2_2).pow(2)
            skip3_diff = (skip3 - skip3_2).pow(2)

            output_diff = self.decoder(fea_diff, skip1_diff, skip2_diff, skip3_diff)
